...
def do_homography(img, H, positions, labels, is_leftfield):
    ...
    # Define label mapping at the start of the function
    label_mapping = {
        "home": 0,
        "away": 1,
        "referee": 2
    ...
    for i, xy in enumerate(positions):
        # Map the string label to an integer using label_mapping
        label_key = labels[i]
        label_int = label_mapping.get(label_key)

        # Raise an error if label is not recognized in label_mapping
        if label_int is None:
            raise ValueError(f"Label '{label_key}' is not recognized in label_mapping.")

        # Perform point transformation and draw the circle with mapped color
        p = np.array([xy[0], xy[1], 1])
        p1 = M @ p
        p1 = p1 / p1[2]
        xy = (int(p1[0]), int(p1[1]))
        cv2.circle(result, xy, 1, colormap[label_int], 10)
...
def drawPlayer(players_position_of_frames, players_position, labels, M, is_leftfield, templateCourtImg):
    # Define label mapping within the function
    label_mapping = {
        "home": 0,
        "away": 1,
        "referee": 2
    ...
    for i, xy in enumerate(players_position):
        ...
        pos_homography = (
            int(m + pos_homography[0] * x_ratio + rightfield_offset),
    ...
    for i, xy in enumerate(players_position_of_frames):
        # Map label to integer using label_mapping
        label_key = labels[i]
        label_int = label_mapping.get(label_key)
        ...
        # Draw the circle using the mapped color
        cv2.circle(templateCourtImg, (xy[0], xy[1]), 4, colormap[label_int], -1)

    # Draw the newest player position
    for i, xy in enumerate(newest_player_position):
        pos_len = len(newest_player_position)
        label_key = labels[-pos_len + i]
        label_int = label_mapping.get(label_key)

        # Raise an error if label is not recognized
        if label_int is None:
            raise ValueError(f"Label '{label_key}' is not recognized in label_mapping.")

        color = tuple(int(0.56 * c) for c in colormap[label_int])
